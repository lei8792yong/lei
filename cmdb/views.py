...
def exportAgencyCustomers_cmdb(request):
    ...
    for cmdbs in cmdb.objects.all():
        ...
        sheet.write(row,0, cmdbs.id)
        sheet.write(row,1, cmdbs.cpu)
        sheet.write(row,2, cmdbs.motherboard)
        sheet.write(row,3, cmdbs.memory)
        sheet.write(row,4, cmdbs.graphics)
        sheet.write(row,5, cmdbs.hard_disk1)
        sheet.write(row,6, cmdbs.keyboard)
        sheet.write(row,7, cmdbs.chassis)
        sheet.write(row,8, cmdbs.power_supply)
        sheet.write(row,9, cmdbs.monitor)
        sheet.write(row,10, cmdbs.who_uses)
